...
export async function GET() {
  try {
    ...
    if (isDbUnavailableError(error)) {
      return NextResponse.json({ rules: defaultRules }, { headers: { "x-data-source": "fallback" } });
    }
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (isDbUnavailableError(error)) {
      return errorJson("Database offline. Rule updates are temporarily unavailable.", 503);
    }
