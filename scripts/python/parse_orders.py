"""
Parse Amazon Orders or Order Items CSV files and insert raw order revenue rows into SQLite.

Assumptions (explicit, minimal):
- The target table already exists and is named "orders" by default.
- Column names are normalized to snake_case from the CSV headers.
- No aggregation is performed: one CSV row becomes one DB row.
- Monetary columns are detected by header name and stored as signed floats.
- Date/time columns are detected by header name and stored as ISO strings when present.
"""
...
MONEY_COL_RE = re.compile(r"(price|tax|amount|discount)", re.IGNORECASE)
...
def parse_orders_csv(
    ...
    """
    Parse an Orders or Order Items CSV and insert rows into SQLite.

    Extracts: order_id, sku, quantity, item_price, item_tax, promotion_discount.
    Stores raw order revenue only (no fees). Does not calculate profit.

    Args:
        csv_path: Path to the Orders or Order Items CSV file.
    ...
    with sqlite3.connect(db_path) as conn:
        if chunksize:
            for chunk in pd.read_csv(csv_path, dtype=str, keep_default_na=False, chunksize=chunksize):
                rows_read += len(chunk)
                cleaned = _clean_orders_frame(chunk)
                rows_inserted += _insert_frame(conn, cleaned, table_name)
        else:
            df = pd.read_csv(csv_path, dtype=str, keep_default_na=False)
            rows_read = len(df)
            cleaned = _clean_orders_frame(df)
            rows_inserted = _insert_frame(conn, cleaned, table_name)

    return ParseResult(rows_read=rows_read, rows_inserted=rows_inserted, table_name=table_name)


def _clean_orders_frame(df: pd.DataFrame) -> pd.DataFrame:
    """
    Normalize headers and enforce required columns for orders.
    """
    df = df.copy()
...
def _is_money_col(col: str) -> bool:
    """
    Heuristic: columns containing price/tax/amount/discount, excluding currency columns.
    """
